    ...
    for root, dirs, files in os.walk(input_path):
        for file in files:
            if file.endswith((".mkv", ".mp4")):
                ...
                output_folder = os.path.join(output_path, filename)  # create output folder path
                os.makedirs(output_folder, exist_ok=True)  # create output folder if not exists
                print(output_path)
                split_video(video_path, output_folder, skip, verbose)  # pass output folder as output path
